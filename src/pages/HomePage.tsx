...
import { getRankedPlayers, GAMEMODES, getPlayerAvatarUrl, TIER_POINTS } from "@/lib/data";
import { TierBadge } from "@/components/TierBadge";
import { Swords, Trophy, Users, ChevronRight } from "lucide-react";

const top5 = getRankedPlayers().slice(0, 5);
    ...
    <div className="min-h-screen">
        ...
        <div className="container mx-auto px-4 relative z-10 text-center">
          ...
          >
            ...
            <div className="flex items-center justify-center gap-4 flex-wrap">
              <Link
                to="/leaderboard"
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-primary text-primary-foreground font-heading font-bold text-lg glow-cyan hover:scale-105 transition-transform"
              >
                <Trophy className="w-5 h-5" /> View Leaderboard
              </Link>
              <Link
                to="/players"
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-secondary text-secondary-foreground font-heading font-bold text-lg hover:bg-secondary/80 transition-colors"
              >
      ...
      <section className="container mx-auto px-4 -mt-8 mb-16">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Players", value: "100+", icon: Users },
      ...
      <section className="container mx-auto px-4 mb-20">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {GAMEMODES.map((gm, i) => (
            ...
            >
              <Link
                to={`/gamemodes?mode=${gm.id}`}
                className="glass-card p-6 text-center hover:glow-cyan hover:border-primary/30 transition-all block group"
              >
